...
def submit1_a():
    ...
    if len(entry_file.get()) > 0:
        ...
        if len(XYZ_return) == 3:
            ...
            temp_text = temp_text + "\n\nPrimary output file : 'profiles.obj'"
            if int(CB2_var.get()) == 1:
                temp_text = temp_text + "\nOnly points file : 'profile_points.csv'"
            if int(CB3_var.get()) == 1:
                temp_text = temp_text + "\nOnly edges file : 'profile_edges.csv'"
...
def OpenContact():
    ...
    stem1.iconbitmap(resource_path("profile_icon.ico"))
    stem1.resizable(0,0)
...
def HelpManual():
    ...
    stem2.title("Manual")
    stem2.iconbitmap(resource_path("profile_icon.ico"))
    stem2.resizable(0,0)
...
if __name__ == '__main__':
    ...
    root.title("Projection-based Terrain Profiler")
    root.iconbitmap(resource_path("profile_icon.ico"))
    #root.state('zoomed')
    root.resizable(0,0)
